...
The experimental Windows backend uses a fresh AppContainer identity for each
command and a kill-on-close Job Object for its descendants. Run it as a standard
Windows user. Build =scripts/build-windows-helper.ps1= from a Visual Studio
SDK developer shell with Clang installed, then ship
...
points, hard-linked files, protected DACL trees, and paths of 240 characters
...
Filesystem grants are temporary SID-specific ACL entries. Invocation execution
and ACL changes are serialized within a Windows session. Keep the policy trees
and their ACLs free of concurrent changes by other host processes during a run.
