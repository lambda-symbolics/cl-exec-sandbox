...
- read-only host and workspace-write presets that hide the user's home
  directory, apart from workspace roots, search-path directories, and the
...
Both presets deny the =:home= special root, where credentials such as SSH
keys, cloud tokens, and tool logins usually live, unless =:hide-home-p= is
false. Deeper rules still apply inside it: workspace roots stay writable, and
=:search-path= and =:working-directory= rules keep the executables on =PATH=
and the command's starting directory readable. Every symbolic link a granted
path passes through is granted too, so a =PATH= entry such as a Nix profile
resolves through its chain of links. Programs that read configuration from
=HOME= need a =HOME= outside the hidden directory; callers should point it at
a private writable directory.
